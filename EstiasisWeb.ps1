...
<script>
 ...
 function cancelDraft(){ toggleMenu(); confirmDlg(L.cancelDraftConfirm, function(){
   ...
   CUR.pending=[]; CUR.sel=null; CUR.msel=[]; CUR.notes=keep; CUR._nraw=nraw;
 ...
 function openTable(id){
   ...
   VIEW='table'; CUR={id:id,desc:t.description,areaId:t.serviceArea_ID,order:null,pending:[],seq:0,sel:null,msel:[],notes:[],level:1,maxLevel:1,lnames:{},seqOk:false,bill:0,maxBill:0,bnames:{},billOk:false,_extChanged:false,_draw:'',_dsaving:0,_draftCoEdited:false};
 ...
 function renderPlates(order){
   ...
   function rowHtml(r){
     if(r.k==='note'){ var ni=r.ni; var ncls=(r.n&&r.n.ok)?'':' ndraft'; return '<div class="pl note'+ncls+(isSelRow('note',ni)?' sel':'')+'" data-k="note" data-ref="'+ni+'" onclick="selectRow(\'note\','+ni+')"><div class="ntext">'+esc(r.n.t)+'</div></div>'; }
     if(r.k==='c'){ var l=r.l; var ed=r.edit;
       ...
       var cls=(Number(l.quantity)||0)<0?' neg':''; if(ed)cls+=' pend'; if(isSelRow('c',l.orderLine_ID))cls+=' sel';
       return '<div class="pl'+cls+'" data-k="c" data-ref="'+l.orderLine_ID+'" onclick="selectRow(\'c\','+l.orderLine_ID+')"><span class="q">'+fmtQ(l.quantity)+'</span><span class="n">'+esc(l.description)+cmt+'</span><span class="v">'+money(ed?ed.Value:l.value)+'</span></div>'+ex;
     }
     var pl=r.l;
     var cmt=pl.Comments?'<div class="cmt">'+esc(pl.Comments)+'</div>':'';
     var cls=' pend'; if((Number(pl.Quantity)||0)<0)cls+=' neg'; if(isSelRow('p',pl._id))cls+=' sel';
     return '<div class="pl'+cls+'" data-k="p" data-ref="'+pl._id+'" onclick="selectRow(\'p\','+pl._id+')"><span class="q">'+fmtQ(pl.Quantity)+'</span><span class="n">'+esc(pl.Description)+cmt+'</span><span class="v">'+money(pl.Value)+'</span></div>';
   }
   var h='';
   if(billsActive){
 ...
 function confirmDelDishes(n){ confirmDlg(L.billDelDishesConfirm, function(){ removeBill(n,'del',0); }); }
 ...
 function openBillPicker(){ if(mselOn()){ var mxm=Math.max(usedMaxBill(),CUR.maxBill||0); var hm=''; for(var bm=0;bm<=mxm;bm++){ var lblm=billLabel(bm); hm+='<button class="billpick" onclick="moveBillTo('+bm+')">'+esc(lblm)+'</button>'; } document.getElementById('bptitle').textContent=L.billMove; document.getElementById('bpcancel').textContent=L.cancel; document.getElementById('bplist').innerHTML=hm; document.getElementById('billsheet').style.display='flex'; return; } var r=selRow(); if(!r){ flash(L.selectItem); return; } var mx=Math.max(usedMaxBill(),CUR.maxBill||0); var cur=(r.k==='note')?((CUR.notes[r.i])?(Number(CUR.notes[r.i].bill)||0):0):((r.k==='p')?(Number(r.l.BillNumber)||0):(Number(r.l.billNumber)||0)); var h=''; for(var b=0;b<=mx;b++){ var lbl=billLabel(b); h+='<button class="billpick'+(b===cur?' cur':'')+'" onclick="moveBillTo('+b+')">'+esc(lbl)+'</button>'; } document.getElementById('bptitle').textContent=L.billMove; document.getElementById('bpcancel').textContent=L.cancel; document.getElementById('bplist').innerHTML=h; document.getElementById('billsheet').style.display='flex'; }
 function closeBillPicker(){ document.getElementById('billsheet').style.display='none'; }
 function moveBillTo(b){ if(mselOn()){ var rows=actionRows(); closeBillPicker(); var committed=[]; var ch=false; var chN=false; rows.forEach(function(r){ if(!r)return; if(r.k==='note'){ if(r.n){ r.n.bill=b; chN=true; } } else if(r.k==='p'){ r.l.BillNumber=b; ch=true; } else { committed.push(r.l); } }); CUR.msel=[]; CUR.sel=null; if(ch)saveDraft(); if(chN)saveNotes(); if(committed.length){ changeCommittedBillMany(committed,b); } else { renderPlates(CUR.order); } return; } var r=selRow(); closeBillPicker(); if(!r)return; if(r.k==='note'){ if(CUR.notes&&CUR.notes[r.i])CUR.notes[r.i].bill=b; CUR.sel=null; saveNotes(); renderPlates(CUR.order); return; } if(r.k==='p'){ r.l.BillNumber=b; CUR.sel=null; saveDraft(); renderPlates(CUR.order); } else { changeCommittedBill(r.l,b); } }
 function changeCommittedBillMany(saveds,b){ var ord=CUR.order; var lines=saveds.map(function(s){ var ln=mkModified(s,Number(s.price)||0,(s.comments||null)); ln.BillNumber=b; return ln; }); var order={Order_ID:ord.order_ID,TradingPeriod_ID:-1,Employee_ID:0,ServiceArea_ID:ord.serviceArea_ID,Table_ID:CUR.id,Guests:0,PriceList_ID:PL.defaultPriceList,Customer_ID:null,CustomerName:null,CustomerDiscount:null,TimeOpen:null,TimeBilled:null,TimePayed:null,Comments:null,LastChanged:null,ExternalId:null,OrderProducts:lines,OrdersOrder:[],TrackingState:0,EntityIdentifier:'00000000-0000-0000-0000-000000000000'};
   flash(L.saving);
   fetch('/api/save?pricelistId='+PL.defaultPriceList,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify([order])})
     .then(function(rr){ if(!rr.ok) throw new Error('save '+rr.status); return rr.text(); })
     .then(function(){ return fetch('/api/order?table='+CUR.id).then(function(x){return x.json();}); })
     .then(function(o){ CUR.order=o; CUR.sel=null; CUR.msel=[]; CUR._osig=orderSig(o); renderPlates(o); })
     .catch(function(e){ alertDlg(L.error+': '+e); renderPlates(CUR.order); }); }
 ...
 function deleteBillStart(n){ if(n==null)n=window.BEIDX; closeBillEdit(); if(!billHasDishes(n)&&!billHasNotes(n)){ removeBill(n,'del',0); return; }
   var mx=Math.max(usedMaxBill(),CUR.maxBill||0); var h='<button class="billpick" onclick="confirmDelDishes('+n+')">'+esc(L.billDelDishes)+'</button>';
 ...
 function selectRow(kind,ref){ if(window._suppressTap){ window._suppressTap=false; return; } if(mselOn()&&(kind==='c'||kind==='p'||kind==='note')){ mselToggle(kind,ref); return; } if(CUR.msel&&CUR.msel.length)CUR.msel=[]; if(CUR.sel&&CUR.sel.kind===kind&&CUR.sel.ref===ref){ CUR.sel=null; } else { CUR.sel={kind:kind,ref:ref}; CUR.level=selLevel(); } renderPlates(CUR.order); }
 ...
 function mselAdd(k,ref){ if(!CUR.msel)CUR.msel=[]; if(!CUR.msel.length&&CUR.sel&&(CUR.sel.kind==='c'||CUR.sel.kind==='p'||CUR.sel.kind==='note'))CUR.msel.push({k:CUR.sel.kind,ref:CUR.sel.ref}); if(mselIndex(k,ref)<0)CUR.msel.push({k:k,ref:ref}); CUR.sel=null; renderPlates(CUR.order); }
 function isSelRow(k,ref){ if(CUR.sel&&CUR.sel.kind===k&&CUR.sel.ref===ref)return true; return mselIndex(k,ref)>=0; }
 function resolveSel(s){ if(s.k==='p'){ var l=findPending(s.ref); return l?{k:'p',l:l}:null; } if(s.k==='c'){ var l=savedLines().filter(function(x){return x.orderLine_ID===s.ref;})[0]; return l?{k:'c',l:l}:null; } if(s.k==='note'){ var nn=(CUR.notes&&CUR.notes[s.ref]!=null)?CUR.notes[s.ref]:null; return nn?{k:'note',i:s.ref,n:nn}:null; } return null; }
 ...
 function mselSameLevel(){ if(!mselOn())return 0; var lv=null; var rows=actionRows(); for(var i=0;i<rows.length;i++){ var r=rows[i]; if(!r)continue; var v=(r.k==='note')?(Number(r.n.seq)||1):((r.k==='p')?(Number(r.l.CourseSeq)||1):(Number(r.l.courseSeq)||1)); if(lv===null)lv=v; else if(lv!==v)return 0; } return lv||0; }
 ...
 function qIncRow(r){ if(r.k==='p'){ r.l.Quantity=(Number(r.l.Quantity)||0)+1; if(r.l.Quantity===0){ removePending(r.l._id); } else { recalc(r.l); } } else { var pid=r.l.product_ID, price=Number(r.l.price)||0; var neg=(CUR.pending||[]).filter(function(l){return !l._edit&&l.Product_ID===pid&&(Number(l.Quantity)||0)<0;})[0]; if(neg){ neg.Quantity=(Number(neg.Quantity)||0)+1; if(neg.Quantity===0){ removePending(neg._id); } else { recalc(neg); } } else { var pos=(CUR.pending||[]).filter(function(l){return !l._edit&&l.Product_ID===pid&&(Number(l.Quantity)||0)>0&&Number(l.Price)===price;})[0]; if(pos){ pos.Quantity=(Number(pos.Quantity)||0)+1; recalc(pos); } else { CUR.pending.push(mkLineFromSaved(r.l,1)); } } } }
 function qDecRow(r){ var pid=(r.k==='p')?r.l.Product_ID:r.l.product_ID; if(effQty(pid)<1){ return false; } if(r.k==='p'){ r.l.Quantity=(Number(r.l.Quantity)||0)-1; if(r.l.Quantity===0){ removePending(r.l._id); } else { recalc(r.l); } } else { var price=Number(r.l.price)||0; var pos=(CUR.pending||[]).filter(function(l){return !l._edit&&l.Product_ID===pid&&(Number(l.Quantity)||0)>0&&Number(l.Price)===price;})[0]; if(pos){ pos.Quantity=(Number(pos.Quantity)||0)-1; if(pos.Quantity===0){ removePending(pos._id); } else { recalc(pos); } } else { var neg=(CUR.pending||[]).filter(function(l){return !l._edit&&l.Product_ID===pid&&(Number(l.Quantity)||0)<0;})[0]; if(neg){ neg.Quantity=(Number(neg.Quantity)||0)-1; recalc(neg); } else { CUR.pending.push(mkLineFromSaved(r.l,-1)); } } } return true; }
 function qInc(){ var act=actionRows().filter(function(r){return r&&r.k!=='note';}); if(!act.length){ flash(L.selectItem); return; } act.forEach(function(r){ qIncRow(r); }); saveDraft(); renderPlates(CUR.order); }
 function qDec(){ var act=actionRows().filter(function(r){return r&&r.k!=='note';}); if(!act.length){ flash(L.selectItem); return; } var any=false; act.forEach(function(r){ if(qDecRow(r))any=true; }); if(any){ saveDraft(); renderPlates(CUR.order); } else { flash(L.cantRemove); } }
 ...
 function commit(){
   ...
   var refresh=function(){ return fetch('/api/order?table='+CUR.id).then(function(r){return r.json();}).catch(function(){return CUR?CUR.order:null;}).then(function(o){ if(!CUR)return true; CUR.order=o; CUR._osig=orderSig(o); CUR._extChanged=false; CUR._draftCoEdited=false; CUR.sel=null; CUR.msel=[]; renderPlates(o); return true; }); };
 ...
 document.addEventListener('click', function(e){ if(!CUR||(!CUR.sel&&!mselOn()))return; var t=e.target; if(t&&t.closest&&(t.closest('.pl')||t.closest('#qbar')||t.closest('#esheet')||t.closest('#sheet')||t.closest('#nsheet')||t.closest('#lnsheet')||t.closest('#billsheet')||t.closest('#billedit')||t.closest('#menu')||t.closest('.lvlhdr')||t.closest('.addlvl')))return; CUR.sel=null; CUR.msel=[]; renderPlates(CUR.order); }, false);
 ...
 function initMultiSelect(){ var p=document.getElementById('plates'); if(!p)return; var LP=null; var clr=function(){ if(LP){ clearTimeout(LP.t); LP=null; } };
   p.addEventListener('pointerdown', function(e){ window._suppressTap=false; var row=(e.target&&e.target.closest)?e.target.closest('.pl[data-ref]'):null; if(!row)return; var k=row.getAttribute('data-k'); if(k!=='c'&&k!=='p'&&k!=='note')return; var ref=parseInt(row.getAttribute('data-ref'),10); LP={k:k,ref:ref,x:e.clientX,y:e.clientY,fired:false}; LP.t=setTimeout(function(){ if(LP&&multiAllowed()){ LP.fired=true; mselAdd(LP.k,LP.ref); } }, 420); }, false);
 ...
 applyLabels(); applyWrapHeight(); initSheet(); initSheetBody(); initMultiSelect(); showTables(); armBack(); setInterval(pollTick, 5000);
</script></body></html>
'@
